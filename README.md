...
一台 bot 可以跟著好幾個儲存庫。討論串的對應表本來就記了 repo,所以兩個 repo 的 #12 不會互相認親。

要注意的是 **webhook 是設在單一 repo 上的**,新的 repo 不會繼承 —— 新專案開起來之後 GitHub → Discord 那半會完全靜音,而且沒有任何錯誤訊息。每個要同步的 repo 都得自己掛一次 webhook,payload URL、secret、事件都一樣。

每個 repo 可以有自己的公告頻道:

```
DEV_ANNOUNCE_CHANNELS=owner/one=123,owner/two=456
```
